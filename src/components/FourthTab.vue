...
<template>
  <div>
    ...
    <b-button class="mt-2 mb-2" variant="success" v-on:click="generateJSON()">Generate app_req JSON</b-button>
    <br>
    <b-button class="mt-2 mb-2" variant="success" v-on:click="call_RE()">Send app_req to RE</b-button>
    <br>
    <b-button class="mt-2 mb-2" variant="success" v-on:click="deploytest_IDEKO()">IDEKO deployment test</b-button>
    <br>
...
export default {
  ...
  data() {
    return {
      ...
      DEresponse: null,
      finalJSON: {},
      //testDeploymentJSON_IDEKO va cancellato dopo i test!
      testDeploymentJSON_IDEKO: {  "ABSTRACT_PROPERTIES": [
        {
  ...
  methods: {
      ...
      deploytest_IDEKO: function() {

        var pippo;

        axios
             .post('http://153.92.30.56:50012/blueprint',
              this.testDeploymentJSON_IDEKO,
              {headers: {'Content-Type': 'application/json'}})
             .then(response => (pippo = response.data))
      ...
      requestDeployment: function(selectedBlueprint) {
        ...
        if (numFound!=1){
          ...
          console.log("Ok, perform post call here...");

          var pippo;

          /*
          // THIS IS THE CORRECT CALL TO THE DEPLOYMENT ENGINE ( IT IS PASSING blueprintObject, THE ONE RECEIVED FROM THE RE)
          axios
               .post('http://153.92.30.56:50012/blueprint',
                blueprintObject,
                {headers: {'Content-Type': 'application/json'}})
          ...
          */
          ...
          // THIS A TEMPORARY TEST CALL TO THE DEPLOYMENT ENGINE ( IT IS SENDING A HARD-CODED JSON WITH THE RIGHT USERNAME/PASSWORD)
          axios
               .post('http://153.92.30.56:50012/blueprint',
                this.testDeploymentJSON_IDEKO,
                {headers: {'Content-Type': 'application/json'}})
